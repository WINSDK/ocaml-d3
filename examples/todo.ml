...
module Model = struct
  ...
  let init = { input = ""; items = []; filter = All; editing = None; toggle = false }
  let change_input t input = { t with input }
  let change_filter t filter = { t with filter }
  ...
  let start_edit t i = { t with editing = Some (fst (List.nth t.items i), i) }
  ...
  let cancel_edit t = { t with editing = None }
  let add_item t = { t with input = ""; items = (t.input, false) :: t.items }
  ...
  let undone t = List.(length (filter (fun (_, b) -> not b) t.items))
  let toggle t = { t with toggle = not t.toggle }
end
...
module View = struct
  ...
  let items k =
    ...
    |- nest enter [ append "li" |. E.dblclick (fun _ _ i -> k (Event.Edit i)) ]
    |- nest
         update
         [ classed "editing" (fun _ m _ -> m.editing)
         ; classed "completed" (fun _ m _ -> m.completed)
         ; static "div"
           |. str attr "class" "view"
           |. seq
                [ static "input"
                  ...
                  |. property "checked" (fun _ m _ -> Js.bool m.completed)
                  |. E.click (fun _ m i ->
                    if m.completed then k (Event.Uncheck i) else k (Event.Check i))
                ; static "label" |. text (fun _ m _ -> m.text)
                ; static "button"
                  |. text (fun _ _ _ -> "destroy")
                  |. str attr "class" "destroy"
                  |. E.click (fun _ _ i -> k (Event.Delete i))
                ]
         ; static "input"
           |. str attr "class" "edit"
           |. property "value" (fun _ m _ -> Js.string m.text)
           |. E.input (fun e _ _ ->
  ...
  let filters k =
    ...
    |- nest update [ select "a" |. classed "selected" (fun _ (_, active, _) _ -> active) ]
    ...
    |- seq
         [ nest
             ...
             ; static "input"
               ...
               |. E.keyup (fun e _ _ -> if e##.keyCode = 13 then k Event.AddInput else ())
             ]
         ; static "section"
           <.> str attr "id" "main"
           |. seq
                ...
                ; static "ul" |. str attr "id" "todo-list" |- items k
                ]
         ; static "footer"
           <.> str attr "id" "footer"
           |. seq
                ...
                ; static "ul" <.> str attr "id" "filters" |- filters k
                ]
         ]
  ;;
  ...
  let low_footer =
    let content =
      ...
      \    <p>Created by <a href='http://computationallyendowed.com'>Spiros \
       Eliopoulos</a></p>\n\
      \    <p>Part of <a href='http://todomvc.com'>TodoMVC</a></p>\n\
      \  "
    in
    static "footer" |. str attr "id" "info" |. html (fun _ _ _ -> content)
  ;;

  let make k = seq [ todoapp k; low_footer ]
end
...
module Storage : sig
  ...
  let storage =
    Optdef.case Dom_html.window##.localStorage (fun () -> None) (fun s -> Some s)
  ;;
  ...
  let get () =
    ...
    | None -> []
    | Some s ->
      (match Opt.to_option (s##getItem key) with
  ...
  let set v =
    match storage with
    | None -> ()
    | Some s -> s##(setItem key (string (Yojson.Basic.to_string (Model.items_to_json v))))
  ;;
end

let main () =
  let model = ref { Model.init with Model.items = Storage.get () } in
  let rec go () = D3.run ~node:Dom_html.document##.body (Lazy.force view) !model
  and view =
    lazy
      (View.make (fun e ->
